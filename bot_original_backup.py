...
async def on_message(message):
    ...
    # Process commands after all other checks
    await bot.process_commands(message)
    ...
    # Delete the message if it contains bad words
...
# Prison Break Game data storage
...
async def prison_break(interaction: discord.Interaction, action: str, user: Optional[discord.Member] = None):
    ...
    try:
        ...
        if action == "start":
            ...
            for channel in interaction.guild.channels:
                if hasattr(channel, 'name') and channel.name == 'jail-cam':
                    jail_cam_channel = channel
                    break
            ...
            if jail_cam_channel:
                ...
                try:
                    for emoji in ["✨", "🤝", "💡", "🔦", "🗝️", "⏰", "📍", "🎯", "🆘"]:  # Help emojis
                        await message.add_reaction(emoji)
                    for emoji in ["😈", "🚨", "🔒", "💥", "🌩️", "📢", "⛔", "💀", "🔥"]:  # Sabotage emojis
                        await message.add_reaction(emoji)
                except Exception as e:
...
async def start_stage_challenge(guild, game_id, stage):
    ...
    if jail_cam_channel:
        ...
        embed.add_field(
            ...
            value="React with emojis below to help (✨🤝💡) or sabotage (😈🚨🔒) the prisoners!",
            inline=True
        )
        
        message = await jail_cam_channel.send(embed=embed)
        
        # Add reaction emojis for spectator interaction
        try:
            # Add a few key help and sabotage emojis for this stage
            for emoji in ["✨", "🤝", "💡"]:  # Help emojis
                await message.add_reaction(emoji)
            for emoji in ["😈", "🚨", "🔒"]:  # Sabotage emojis
                await message.add_reaction(emoji)
        except Exception as e:
            print(f"Error adding reactions to stage challenge: {e}")
        
        # Log to mod-logs
        log_channel = discord.utils.get(guild.text_channels, name="mod-logs")
        if log_channel:
            embed = discord.Embed(
                title=f"Prison Break Stage {stage} Started",
                description=f"New stage: {PRISON_BREAK_STAGES[stage]['name']}\n{PRISON_BREAK_STAGES[stage]['description']}",
                color=discord.Color.blue(),
                timestamp=datetime.datetime.now(UTC)
            )
            embed.add_field(name="Stage", value=f"**{stage}**", inline=True)
            embed.add_field(name="Time Limit", value=f"{PRISON_BREAK_STAGES[stage]['time_limit']} seconds", inline=True)
            embed.add_field(name="Spectator Help", value=PRISON_BREAK_STAGES[stage]['spectator_help'], inline=True)
            await log_channel.send(embed=embed)
            
            # Start the stage timer
            game_data["stage_start_time"] = str(datetime.datetime.now(UTC))
        else:
            print(f"No mod-logs channel found for stage {stage}")
        
    except Exception as e:
        print(f"Error starting stage challenge: {e}")

@bot.tree.command(name="help", description="Help the prisoners in their escape attempt")
async def help_prisoners(interaction: discord.Interaction):
    guild_id = str(interaction.guild.id)
    
    # Check if user is quarantined (quarantined users can't help)
    if guild_id in quarantine_data and str(interaction.user.id) in quarantine_data[guild_id]:
        await interaction.response.send_message("❌ Quarantined prisoners can't help themselves externally! Use your prison skills instead!", ephemeral=True)
        return
        
    # Find active prison break games
    if guild_id not in prison_break_data:
        await interaction.response.send_message("❌ No prison break games active!", ephemeral=True)
        return
    ...
    if not active_games:
        await interaction.response.send_message("❌ No active prison break games to help with!", ephemeral=True)
        return
        
    # Add help vote to the first active game
    game_id, game_data = active_games[0]
    game_data["spectator_votes"]["help"] += 1
    
    # Determine help effect based on current challenge
    stage = game_data["stage"]
    current_challenge = game_data.get("current_challenge")
    
    help_messages = [
        f"🤝 {interaction.user.display_name} threw a rope ladder to the prisoners!",
        f"💡 {interaction.user.display_name} whispered helpful hints through the bars!",
        f"🔦 {interaction.user.display_name} provided a flashlight for better visibility!",
        f"🗝️ {interaction.user.display_name} slipped lock picking tools under the door!",
        f"⏰ {interaction.user.display_name} created a distraction, buying more time!",
        f"📍 {interaction.user.display_name} pointed out the guard patrol routes!",
        f"🎯 {interaction.user.display_name} gave tactical advice from outside!"
    ]
    
    help_message = random.choice(help_messages)
    
    # Provide actual help based on stage
    hint = ""
    if stage == 1 and current_challenge:
        hint = f"\n💡 **Hint:** {current_challenge['hint']}"
    elif stage == 2 and current_challenge:
        remaining = len(current_challenge['answer']) - len(current_challenge.get('progress', []))
        hint = f"\n💡 **Hint:** {remaining} more directions needed! Try going in different directions."
    elif stage == 3 and current_challenge:
        hint = f"\n💡 **Hint:** Look for places where {current_challenge['guards']} guards can't see you!"
    elif stage == 4 and current_challenge:
        hint = f"\n💡 **Hint:** The code word starts with '{current_challenge['code_word'][0]}'!"
    ...
    if jail_cam_channel:
        await jail_cam_channel.send(f"🆘 **PRISONER ASSISTANCE!**\n{help_message}{hint}")
        
    await interaction.response.send_message("✅ You helped the prisoners! Your assistance has been noted.", ephemeral=True)

@bot.tree.command(name="sabotage", description="Sabotage the prisoners' escape attempt")
async def sabotage_prisoners(interaction: discord.Interaction):
    guild_id = str(interaction.guild.id)
    
    # Check if user is quarantined (quarantined users can't sabotage)
    if guild_id in quarantine_data and str(interaction.user.id) in quarantine_data[guild_id]:
        await interaction.response.send_message("❌ Quarantined prisoners can't sabotage from inside! Focus on your own escape!", ephemeral=True)
        return
        
    # Find active prison break games
    if guild_id not in prison_break_data:
        await interaction.response.send_message("❌ No prison break games to sabotage!", ephemeral=True)
        return
    ...
    if not active_games:
        await interaction.response.send_message("❌ No active prison break games to sabotage!", ephemeral=True)
        return
    ...
    sabotage_messages = [
        f"😈 {interaction.user.display_name} alerted the guards to suspicious activity!",
        f"🚨 {interaction.user.display_name} triggered the alarm system!",
        f"🔒 {interaction.user.display_name} jammed the lock mechanism!",
        f"💥 {interaction.user.display_name} caused a distraction that brought more guards!",
        f"🌩️ {interaction.user.display_name} cut the power, making everything harder!",
        f"📢 {interaction.user.display_name} made noise to attract guard attention!",
        f"⛔ {interaction.user.display_name} blocked the escape routes!"
    ]
    ...
    if jail_cam_channel:
        await jail_cam_channel.send(f"😈 **SABOTAGE DETECTED!**\n{sabotage_message}\n🔥 The escape just got harder!")
        
    await interaction.response.send_message("😈 You sabotaged the prisoners! Chaos ensues.", ephemeral=True)
...
async def advance_prison_break_stage(guild, guild_id, game_id, game_data):
    ...
    try:
        ...
        if current_stage < 4:
            ...
            if log_channel:
                embed = discord.Embed(
                    title=f"Prison Break Stage {next_stage} Started",
                    description=f"New stage: {PRISON_BREAK_STAGES[next_stage]['name']}\n{PRISON_BREAK_STAGES[next_stage]['description']}",
                    color=discord.Color.blue(),
                    timestamp=datetime.datetime.now(UTC)
                )
                embed.add_field(name="Stage", value=f"**{next_stage}**", inline=True)
                embed.add_field(name="Time Limit", value=f"{PRISON_BREAK_STAGES[next_stage]['time_limit']} seconds", inline=True)
                embed.add_field(name="Spectator Help", value=PRISON_BREAK_STAGES[next_stage]['spectator_help'], inline=True)
                await log_channel.send(embed=embed)
            ...
            if log_channel:
                embed = discord.Embed(
                    title="Prison Break Game Completed",
                    description=f"The prison break game has been completed! Congratulations to the prisoners!",
                    color=discord.Color.green(),
                    timestamp=datetime.datetime.now(UTC)
                )
                embed.add_field(name="Stage", value=f"**{game_data['stage']}**", inline=True)
                embed.add_field(name="Time Taken", value=f"{datetime.datetime.now(UTC) - datetime.datetime.fromisoformat(game_data['stage_start_time'])}", inline=True)
                embed.add_field(name="Challenges Completed", value=f"{', '.join(game_data['challenges_completed'])}", inline=True)
                await log_channel.send(embed=embed)
                ...
                for player_id in game_data["players"]:
                    ...
                    if member:
                        await member.send(f"The prison break game has ended! You completed stage **{game_data['stage']}** in {datetime.datetime.now(UTC) - datetime.datetime.fromisoformat(game_data['stage_start_time'])}")
                
                # Send final message to spectators
                if game_data["spectator_votes"]["help"] > game_data["spectator_votes"]["sabotage"]:
                    await guild.text_channels.send(f"🎉 Congratulations! The prisoners have successfully completed the prison break game! Spectators helped a lot!")
                elif game_data["spectator_votes"]["sabotage"] > game_data["spectator_votes"]["help"]:
                    await guild.text_channels.send(f"💥 Oh no! The prisoners were caught! Spectators were sabotaging.")
                else:
                    await guild.text_channels.send(f"🎉 Congratulations! The prisoners have successfully completed the prison break game! It was a close call!")
                
                # Reset game data
                game_data["players"] = []
                game_data["stage"] = 1
                game_data["spectator_votes"] = {"help": 0, "sabotage": 0}
                game_data["challenges_completed"] = []
                game_data["current_challenge"] = None
                game_data["stage_start_time"] = None
                game_data["last_activity"] = None
                game_data["attempts"] = {}
...
@bot.tree.command(name="status", description="Check the status of an ongoing prison break game")
@app_commands.describe(
    game_id="The ID of the game to check status for"
)
@app_commands.default_permissions(administrator=True)
async def prison_break_status(interaction: discord.Interaction, game_id: str):
    try:
        # Check if game exists
        if game_id not in prison_break_data or game_id not in prison_break_data[guild_id]:
            await interaction.response.send_message("❌ No such game found!", ephemeral=True)
            return
        ...
        start_time = datetime.datetime.fromisoformat(game_data["start_time"])
        current_time = datetime.datetime.now(UTC)
        time_taken = current_time - start_time
        
        # Calculate remaining time
        stage_info = PRISON_BREAK_STAGES[game_data["stage"]]
        time_left = stage_info["time_limit"] - time_taken.seconds
        
        # Check if time is up
        if time_left <= 0:
            await interaction.response.send_message("⏳ Time's up! The game has ended.", ephemeral=True)
            return
            
        # Calculate progress
        progress = (time_taken.seconds / stage_info["time_limit"]) * 100
        
        # Create embed
        embed = discord.Embed(
            title=f"Prison Break Game Status: {game_id}",
            description=f"Current stage: **{PRISON_BREAK_STAGES[game_data['stage']]['name']}**\n{PRISON_BREAK_STAGES[game_data['stage']]['description']}",
            color=discord.Color.blue()
        )
        embed.add_field(name="Time Taken", value=f"{time_taken} (approx.)", inline=True)
        embed.add_field(name="Time Left", value=f"{datetime.timedelta(seconds=time_left)}", inline=True)
        embed.add_field(name="Progress", value=f"{progress:.2f}%", inline=True)
        
        # Add current challenge details if available
        current_challenge = game_data.get("current_challenge")
        if current_challenge:
            embed.add_field(name="Current Challenge", value=current_challenge["challenge"], inline=False)
            embed.add_field(name="Hint", value=current_challenge["hint"], inline=False)
...
async def on_reaction_add(reaction, user):
    ...
    if not (hasattr(reaction.message.channel, 'name') and reaction.message.channel.name == 'jail-cam'):
        return
