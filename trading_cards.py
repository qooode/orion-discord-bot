...
class TradingCards(commands.Cog):
    ...
    async def daily_card(self, ctx):
        ...
        if image_embed:
            ...
            await ctx.send("✅ Daily card claimed but no image available!")
    ...
    async def card_info(self, ctx, *, card_name: str):
        ...
        if image_embed:
            ...
            # Fallback if no image
            await ctx.send(f"ℹ️ **{card[1]}** - No image available for this card.")
    ...
    async def manual_drop(self, ctx, channel: discord.TextChannel = None, *, card_name: str = None):
        ...
        if image_embed:
            ...
            await channel.send(f"⚠️ **{card[1]}** dropped but no image available!")
            return
